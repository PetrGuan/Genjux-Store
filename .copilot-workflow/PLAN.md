...
Phase 1：**macOS**（AppKit，纯命令式），做完整 GUI（走本地 HTTP API 调 core）。
Phase 2：**Windows**（WinUI 3），复用同一 core 服务。
Phase 3：**Android**（Kotlin + Jetpack Compose，core 经 UniFFI 内嵌），信任/安装模型与桌面端差异最大，故排在桌面两端之后。
Phase 4：**Linux**（GTK4 + libadwaita，可用 gtk-rs 直接在 core 语言内写 UI，是成本最低的一个平台，放最后开发不影响成本优势）。
...
2. ~~首发平台~~：已确认，开发顺序为 **macOS → Windows → Android → Linux**。
